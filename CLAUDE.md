...
- **크롤링**: Jsoup 1.17.2, Firecrawl CLI (사이트 분석용)
...
### 사이트 분석 도구 (Firecrawl CLI + 스킬)

새 크롤러 개발 시 사이트 구조를 분석하는 워크플로우:
1. `firecrawl map <url>` — 사이트 URL 구조 탐색, 카테고리/API 엔드포인트 발견
2. `firecrawl scrape <url> --only-main-content` — 페이지 콘텐츠 추출 (JS 렌더링 포함)
3. `curl` — raw HTML/JSON 응답으로 CSS 셀렉터, API 스키마 확인
4. `firecrawl browser` — 로그인/인터랙션 필요 시 브라우저 자동화

사용 가능한 스킬 (`.claude/skills/`):
- **firecrawl 계열** (8개): scrape, map, crawl, search, agent, browser, download, 통합
- **브라우저 자동화** (7개): agent-browser, browser-use, remote-browser, electron, slack, dogfood, vercel-sandbox
