...
@Component
public class TqueensCrawler implements CampaignCrawler {

    private static final Logger log = LoggerFactory.getLogger(TqueensCrawler.class);
    private static final String API_URL = "https://tqueens.net/taste/taste_list";
    private static final String BASE_URL = "https://tqueens.net";
    ...
    public TqueensCrawler(CrawlingProperties properties, CrawlingDelayHandler delayHandler,
        ...
        for (int page = 1; page <= properties.getMaxPagesPerSite(); page++) {
            try {
                ...
                if (response.statusCode() != 200) {
                    log.error("TQUEENS API 응답 오류: HTTP {}", response.statusCode());
                    break;
                }
                ...
                for (Element card : cards) {
                    try {
                        ...
                        log.warn("TQUEENS 아이템 파싱 실패: {}", e.getMessage());
                    }
                }
                ...
                log.error("TQUEENS 페이지 {} 크롤링 실패: {}", page, e.getMessage());
                break;
            }
        }

        results = enricher.enrich(results, this::parseDetailPage);
        log.info("TQUEENS 크롤링 완료: {}건", results.size());
        return results;
    }
    ...
    private CrawledCampaign parseDetailPage(CrawledCampaign campaign, Document doc) {
        ...
        return new CrawledCampaign(
                ...
                coalesce(campaign.getRecruitCount(), recruitCount), coalesce(campaign.getApplyStartDate(), applyStartDate),
    ...
    private List<CrawledCampaign> generateMockData(CrawlingSource source) {
        ...
        for (int i = 1; i <= properties.getMockCount(); i++) {
            ...
            mocks.add(new CrawledCampaign(
                    source.getCode(), "tqueens-" + i,
                    "[택배의여왕] 체험단 캠페인 #" + i,
                    "택배의여왕 체험단 설명 " + i, null,
                    "https://placehold.co/300x200?text=TQUEENS+" + i,
